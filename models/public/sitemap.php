...
function xmlsf_get_priority( $sitemap = 'post_type', $term = '' ) {
	...
	if ( 'post_type' == $sitemap ) :
		...
		if ( $priority_meta = get_metadata( 'post', $post->ID, '_xmlsf_priority', true ) ) {
			...
			if ( $post->comment_count && $priority < 1 && $pt_comment_count = wp_count_comments($post->post_type)->approved ) {
				$priority += 0.1 + ( 1 - $priority ) * $post->comment_count / $pt_comment_count;
			}
		}
