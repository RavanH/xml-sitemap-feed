...
class BWT_Connect {
	...
	public static function get_property_url( $min_level = 'siteFullUser' ) {
		// Get access token.
		$access_token = \XMLSF\BWT_Connect::get_access_token();
		...
		if ( empty( $properties ) ) {
			return new WP_Error(
				'no-valid-site-url',
				\__( 'Could not get a valid property from Bing Webmaster Tools. The connected user account may not have the correct permissions.', 'xml-sitemap-feed' )
			);
		}
