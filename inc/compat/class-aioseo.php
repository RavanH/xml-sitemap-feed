...
class AIOSEO {
	...
	public static function admin_notices() {
		if ( ! \current_user_can( 'manage_options' ) || \in_array( 'aioseop_sitemap', (array) \get_user_meta( \get_current_user_id(), 'xmlsf_dismissed', false ), true ) ) {
			return;
		}
			...
			<div class="notice notice-warning fade is-dismissible">
				<p>
					...
					\printf( /* translators: Sitemap page name (linked to SEOPress plugin settings), XML Sitemap Index, Reading Settings admin page (linked to Reading settings) */
						...
						'<a href="' . \esc_url( \admin_url( 'options-reading.php' ) ) . '#xmlsf_sitemaps">' . \esc_html( \translate( 'Reading Settings' ) ) . '</a>' // phpcs:ignore WordPress.WP.I18n.LowLevelTranslationFunction
					);
					?>
				</p>
				<form action="" method="post">
					...
					<p>
						...
						<input type="submit" class="button button-small" name="xmlsf-dismiss-submit" value="<?php echo \esc_html( \translate( 'Dismiss' ) ); ?>" />  // phpcs:ignore WordPress.WP.I18n.LowLevelTranslationFunction
					</p>
				</form>
			</div>
			<?php
