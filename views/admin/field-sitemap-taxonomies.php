<fieldset id="xmlsf_taxonomies">
    ...
    <?php if ( empty($taxonomies) || count($tax_list) == count($taxonomies) ) { ?>
	<p>
	    <label>
	        ...
	        <?php printf( /* translators: taxonomies */ __('Include taxonomies'), 'xml-sitemap-feed' ); ?> (<?php echo count($tax_list); ?>)
	    </label>
	</p>
	<?php } else {
		echo implode( '<br/>', $tax_list );
	} // $include_option : implode( '<br/>', $tax_list ); ?>

	<?php if ( empty( $tax_list ) ) { ?>
