...
<div style="border: 4px solid #2a8c41; padding: 0 10px; background-color:lemonchiffon">
	...
	<p>
		<strong>
			<?php esc_html_e( 'Instantly notify Google by automatically resubmitting your Google News sitemap upon each new publication.', 'xml-sitemap-feed' ); ?>
			<?php
			printf(
				/* translators: %s: Advanced plugin name (linked to https://premium.status301.com/) */
				esc_html__( 'Available in %s.', 'xml-sitemap-feed' ),
				'<a href="https://premium.status301.com/downloads/google-news-advanced/" target="_blank">' . __( 'Google News Sitemap', 'xml-sitemap-feed' ) . '</a>'
			);
			?>
		</strong>
	</p>
